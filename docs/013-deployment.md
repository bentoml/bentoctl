...
- Bento build built with BentoML version 0.13 or below.


## Deploy IrisClassifier from BentoML quick start guide to AWS Lambda

1. Setup the AWS Lambda deployment

    a. Clone the operator repo
        ```bash
        git clone https://github.com/bentoml/aws-lambda-deploy.git
        ```

    b. Install the operator requirements
        ```bash
...
2. Copy and change the sample config file given and change it according to your deployment specifications.
    ```
    # lambda_config.json
    {
        "region": "us-west-1",

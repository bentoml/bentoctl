...
import bentoctl.operator.operator as op
...
def test_import_module():
    testop = op._import_module("testop", TESTOP_PATH)
    ...
    with pytest.raises(ModuleNotFoundError):
        testop = op._import_module("MODULE_THAT_IS_NOT_PRESENT", TESTOP_PATH)
...
def test_operator_class_init(tmp_path, monkeypatch):
    ...
    operator = op.Operator(testop_path)
    ...
    with pytest.raises(OperatorLoadException):
        ...
        monkeypatch.setattr(op, "_import_module", raise_import_error)
